...
public:
	...
	void entering_events(std::vector<BButton_ptr> & buttons);
	void leaving_events(std::vector<BButton_ptr> & buttons);

	void events_buttons(std::vector<BButton_ptr> & buttons);
	void render_buttons(std::vector<BButton_ptr> & buttons, const int & render_fromY);

	void set_startPOS(int start);
	void set_endPOS(int end);
private:
	void move_buttons(std::vector<BButton_ptr> & buttons, int size);
private:
...
public:
	...
	void operator=(const std::vector<BButton_ptr> & Buttons);
protected:
...
private:
	std::vector<BButton_ptr> buttons;
