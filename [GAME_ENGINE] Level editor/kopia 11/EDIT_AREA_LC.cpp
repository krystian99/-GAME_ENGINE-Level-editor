		...
		&& mY >= position.y && mY <= position.y + position.h)
	{
		if (edit_state == EditArea_LC_state::PLACING_OBJ) {
			if (Mouse::get_lButton_State() && !lBT_clicked) {
				objects_test.push_back(OBJ_TEST{ mX, mY, int(objects_test.size()) }); // po³ó¿ obiekt wzglêdem obszaru edycji
				lBT_clicked = true;
			}
...
EDIT_AREA_LC::OBJ_TEST::OBJ_TEST(int x1, int y1, int index1) :
	x{ x1 }, y{ y1 }, index{ index1 }
{
	position = { x, y, w, h };
}
...
void EDIT_AREA_LC::OBJ_TEST::setID(int id)
{
	index = id;
}
