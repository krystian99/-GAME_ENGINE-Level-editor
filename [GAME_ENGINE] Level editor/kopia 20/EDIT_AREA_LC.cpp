...
void EDIT_AREA_LC::on_buttonL_MouseClick(const int & mX, const int & mY)
{
	...
	case EditArea_LC_state::PLACING_OBJ:
		...
		createOBJ(x, y, renderScaleObj_X, renderScaleObj_Y);
		break;
	}
}
...
OBJ_TEST::OBJ_TEST(const int & x1, const int & y1, const double & scaleX, const double & scaleY) :
	x{ x1 }, y{ y1 }, renderScaleX{ scaleX }, renderScaleY{ scaleY }
{

	position = { x, y, w, h };

	render_position.x = position.x * renderScaleX;
	render_position.y = position.y * renderScaleY;
	render_position.w = position.w;
	render_position.h = position.h;

	data = { x, y, renderScaleX, renderScaleY };
}
...
void Map_area::setView(std::list<OBJ_TEST>& objects)
{
	static int absolute;
	static int size;
