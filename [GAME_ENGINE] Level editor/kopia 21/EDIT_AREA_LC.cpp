...
void EDIT_AREA_LC::setRenderPOS_enemies()
{
...
void EDIT_AREA_LC::createOBJ(int mX, int mY, const double & scaleX, const double & scaleY)
{
	static int RenderPOS_X, RenderPOS_Y;

	RenderPOS_X = position.w * scaleX + position.x;
	RenderPOS_Y = position.h * scaleY + position.y;

	objects_test.push_back({ mX, mY, scaleX, scaleY }); // po³ó¿ obiekt wzglêdem obszaru edycji
...
void EDIT_AREA_LC::on_buttonL_MouseClick(const int & mX, const int & mY)
{
	...
	case EditArea_LC_state::PLACING_OBJ:
		...
		x = map_renderArea.getPOS().x + map_renderArea.getPOS().w * scaleTX_w;
		y = map_renderArea.getPOS().y + map_renderArea.getPOS().h * scaleTX_h;

		double scaleW = double(x - map_renderArea.getPOS().x) / double(map_renderArea.getPOS().w);
		double scaleH = double(y - map_renderArea.getPOS().y) / double(map_renderArea.getPOS().h);

		//renderScaleObj_X = double(mX) / double(x); // coœ trzeba zmieniæ
		//renderScaleObj_Y = double(mY) / double(y);

		createOBJ(x, y, scaleW, scaleH);
		break;
	}
}
...
{
	//render_position.x = posX;
	//render_position.y = posY;
