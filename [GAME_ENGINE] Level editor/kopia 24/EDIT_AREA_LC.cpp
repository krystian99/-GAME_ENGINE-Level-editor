...
void EDIT_AREA_LC::setMap_view()
{
	...
	static int absRenderX, absRenderY;
		...
		|| Mouse::getMousePOSy() < Mouse::getRecent_y() || Mouse::getMousePOSy() > Mouse::getRecent_y()) 
	{
		...
		absRenderX = abs(position.x + round(position.w * scaleX) - (position.x + round(position.w * scale_recentX)));
		absRenderY = abs(position.y + round(position.h * scaleX) - (position.y + round(position.h * scale_recentX)));
		...
		if (Mouse::getMousePOSx() < Mouse::getRecent_x() && map_renderArea.canMove_r()) {
			if (map_renderArea.area.x + maprender_absX + map_renderArea.area.w < mapBG.getWidth())
				...
				int maprecentX = map_renderArea.area.x;
				map_renderArea.area.x = mapBG.getWidth() - map_renderArea.area.w;
				maprender_absX = abs(maprecentX - map_renderArea.area.x);

				double testX = double(-maprender_absX + map_renderArea.area.w) / double(map_renderArea.area.w);
				
				absRenderX = abs((position.x + round(position.w * testX)) - (position.x + position.w));
			}
			for (auto & enemy : objects_test)
				enemy.move_renderX(-absRenderX);

			// stare - wszystko w tym jednym if'ie
			/*map_renderArea.area.x += maprender_absX;
			for (auto & enemy : objects_test)
				enemy.move_renderX(-absRenderX);*/
		}
...
void EDIT_AREA_LC::createOBJ(const int & X, const int & Y)
{
	...
	objects_test.push_back({ X, Y }); // po³ó¿ obiekt wzglêdem obszaru edycji
