...
void EDIT_AREA_LC::Init()
{
	setPOS(); // ustala pozycjê
...
void EDIT_AREA_LC::renderBackground()
{
	mapBG.render(&map_renderArea.getPOS(), &position);
}

void EDIT_AREA_LC::render()
{
	renderBackground();
	renderEnemies();
	renderBlocks();
	...
	render_eventsDependent(); // aktualizuje stan zdarzen i renderuje - tzw. renderowanie zale¿ne od zdzarzeñ
}

void EDIT_AREA_LC::render_eventsDependent()
{
	...
	if (edit_state == EditArea_LC_state::PLACING_OBJ) {
		...
		test_objectTX.render(nullptr, &mouse_updateDraw);
	}
...
void EDIT_AREA_LC::placingObjects_events()
{
	int mX = Mouse::getMousePOSx();
	int mY = Mouse::getMousePOSy();
...
void EDIT_AREA_LC::movingOBJ_events()
{
	int mX = Mouse::getMousePOSx();
	int mY = Mouse::getMousePOSy();
...
void EDIT_AREA_LC::renderEnemies()
{
...
void EDIT_AREA_LC::renderBlocks()
{
