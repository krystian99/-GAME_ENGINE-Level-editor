...
{
	background.render(nullptr, &barPOS);
	buttons_render();
}
...
void ScrollBar::buttons_render()
{
	for (auto & bt : buttons)
		buttons_render_vertical(bt);
}

void ScrollBar::buttons_render_vertical(Button_LC & bt) // b³¹d - jeden przycisk wgl siê nie renderuje
{
