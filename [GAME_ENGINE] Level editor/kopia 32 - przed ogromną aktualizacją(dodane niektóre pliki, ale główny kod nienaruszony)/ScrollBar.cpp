...
ScrollBar::ScrollBar(const SDL_Rect & pos, const std::string & texture_path) :
	ScrollBar(pos)
{
...
ScrollBar::ScrollBar(const SDL_Rect & pos, const SDL_Color & color) :
	ScrollBar(pos)
{
...
ScrollBar::ScrollBar(const SDL_Rect & pos)
{
	...
	if (position.w > position.h) {
		...
		button_height = button_width = position.h - 2 * button_space;
		//button_width = button_height; // kwadraty
	}
	else {
		orient = ScrollBar_orient::VERTICAL;
		button_width = button_height = position.w - 2 * button_space; // dlatego 2 razy bo przy lewej i prawej krawêdzi ma byæ taki sama pusta przestrzeñ
...
void ScrollBar::render_buttons()
{
	...
	case ScrollBar_orient::HORIZONTAL: // nie zosta³o ukoñczone
		for (auto & button : buttons) {
			...
			if (pos.x >= position.x + button_space) 
				button->render(); // renderuj ca³y przycisk
			else if (pos.x < position.x + button_space) {
				temp_pos = { position.x + button_space, pos.y, abs(pos.x + pos.w - (position.x + button_space)), pos.h };

				auto & tX = button->getTX();

				double scale = double(temp_pos.w) / double(pos.w);
				
				texture_area.x = std::round(tX.getWidth() * scale);
				texture_area.y = 0;
				texture_area.w = tX.getWidth() - texture_area.x;
				texture_area.h = tX.getHeight();
			}
			else if (pos.x > position.x + position.w - button_space) {
