...
void Map_EditArea_manager::set_temp_renderOBJ()
{
	...
	case OBJ_type::ENEMY:
		switch (get_EnemyID()) {
		case Enemy_ID::TEST:
			scaleX = double(Enemy_Test::W) / double(Map_EditArea_manager::get_mapBG_W());
			scaleY = double(Enemy_Test::H) / double(Map_EditArea_manager::get_mapBG_H());

			temp_renderOBJ.w = round(scaleX * Map_EditArea_manager::get_editW());
			temp_renderOBJ.h = round(scaleY * Map_EditArea_manager::get_editH());
			break;
		case Enemy_ID::TEST2:
			scaleX = double(Enemy_Test2::W) / double(Map_EditArea_manager::get_mapBG_W());
			scaleY = double(Enemy_Test2::H) / double(Map_EditArea_manager::get_mapBG_H());

			temp_renderOBJ.w = round(scaleX * Map_EditArea_manager::get_editW());
			temp_renderOBJ.h = round(scaleY * Map_EditArea_manager::get_editH());
			break;
		case Enemy_ID::TEST3:
			scaleX = double(Enemy_Test3::W) / double(Map_EditArea_manager::get_mapBG_W());
			scaleY = double(Enemy_Test3::H) / double(Map_EditArea_manager::get_mapBG_H());
