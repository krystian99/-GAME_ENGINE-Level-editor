...
Map_EditArea::Map_EditArea(const SDL_Rect & pos)
{
	...
	Map_EditArea_manager::set_edit_area(edit_area.w, edit_area.h);
}
...
void Map_EditArea::set_background(const std::string & bg)
{
	...
	Map_EditArea_manager::update_mapBG(mapBG_area.w, mapBG_area.h);*/
...
void Map_EditArea::update_ObjectsSize()
{
...
void Map_EditArea::create_EnemyOBJ(const int & X, const int & Y, const Enemy_ID & id, const SDL_RendererFlip & flip)
{
	...
	auto & map_pos = enemies.back()->get_mapPOS();

	scaleX = double(map_pos.w) / double(mapBG_area.w);
	scaleY = double(map_pos.h) / double(mapBG_area.h);
...
void Map_EditArea::create_EnemyOBJ(const SDL_Rect & mapPOS, const Enemy_ID & id, const SDL_RendererFlip & flip)
{
	using namespace Enemy_Types;
