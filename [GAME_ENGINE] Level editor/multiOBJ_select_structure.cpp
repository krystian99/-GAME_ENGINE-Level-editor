...
void multiOBJ_select_structure::selectingOBJs()
{
	...
	if (Mouse::moved()) {
		...
		selected_area.set_mapPOS(x, y, w, h, edit_area, mapBG_area);
	}
}
...
void multiOBJ_select_structure::on_mouseL1hit()
{
	...
	case States::SELECTING_OBJs:
		...
		clicked_point = { Mouse::getX(), Mouse::getY() };
...
void multiOBJ_select_structure::movingFinsh_event()
{
	...
	for (auto& moveOBJ : moving_objects)
		moveOBJ.enemy->set_mapPOS(edit_area, mapBG_area);

	selected_area.set_mapPOS(edit_area, mapBG_area);
}
...
{
	selected_area.set_renderPOS(edit_area, mapBG_area);
}
